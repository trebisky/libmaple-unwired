/* Driver for an SSD1306 OLED display.
 *
 * The following code has been adapted to work with libmaple-unwired.
 * I found it someplace online, while looking for code in plain C.
 * Apparently this code has been further developed by Adafruit,
 *  transformed into C++ and made a part of a "GFX library".
 * It was nice to be able to find this plain and simple C code
 *  and use it for a starting point.
 *
 * Tom Trebisky  5-17-2020
 *
 * The SSD1306 is a monochrone OLED display with 128 by 64 pixels.
 * The display itself is 0.96 inches, or so they say.
 * I measure 1 inch wide by 1/2 inch tall.
 * I have two sorts of displays.  One is all the same color.
 * The other is yellow on the top 16 pixels and blue below.

 * So, what is the API for this "library" ??
 * Some prototypes for some of the exported functions is as follows:

void ssd_init ( struct i2c * );
void ssd_set_pixel ( int, int );
void ssd_clear_pixel ( int, int );
void ssd_draw_pixel ( int, int, int );
void ssd_clear_all ( void );
void ssd_set_textsize ( int );
void ssd_set_wrap ( int );
void ssd_putc ( int );
void ssd_xy ( int, int );
void ssd_puts ( char * );
...
You call the init function once to hand it the i2c device pointer.
Then you set and clear pixels, or the whole display,
or you write text to it.

0,0 is the upper left corner, which is sensible

Text is 7 pixels high, with a blank line of pixels between lines,
 so a line uses 8 pixels in height.
The default "textsize" is 1, so you get 8 pixel high lines
 and 21 characters per line, and 8 of those lines on the display.
Lines do wrap if you go beyond 21 characters.
You can call ssd_set_wrap ( 0 ) to disable this.
Characters use 6 pixels in width (they are 5 pixels wide, and
  a pixel for space.)

If you set textsize to 2, it doubles the text size, so you get
 16 pixels high line and 10 characters on a line.
Note that if you turn wrap off, you will get part of a character 11.

To position the start of a string, you can call ssd_xy ( x, y )
 or (as a convenience) call ssd_text ( x, y, msg );
Note that these are pixel coordinates, so if you want to think in
 terms of lines, you will have to do some math.

Note that nothing happens until you call ssd_display(), up to then
all the action is going on in a local buffer.
The call to ssd_display() actually writes to the device.

One way to use my split color displays is to use the top 16 yellow
 pixels for a single line of text in size 2 (only 10 chars wide).
Then start at y=16 to access the blue part.
This could be 3 lines of 16 high text (size 2)
 or a single line of size 4 and a single of size 2
 or two lines of size 3 (24 pixels high each)
*/
